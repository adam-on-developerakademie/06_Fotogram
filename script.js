...
function overlayOnOff(imageName) {
    if (imageName != null) { overlayOn(imageName) } else {
        ...
        if (document.getElementById('photoId') != null) { overlayOff() };
    };
    currentImageName = imageName;
}
function overlayOn(imageName) {
...
function clickLeft(event) {
    ...
    if (photoArry.indexOf(currentImageName) == 0) { currentImageName = photoArry[photoArry.length - 1]; overlayOnOff(currentImageName); } else {
        currentImageName = photoArry[photoArry.indexOf(currentImageName) - 1]; overlayOnOff(currentImageName);
    };
...
function clickRight(event) {
    event.stopPropagation();
    if (photoArry.indexOf(currentImageName) == photoArry.length - 1) { currentImageName = photoArry[0]; overlayOnOff(currentImageName); } else {
